import React, { useEffect, useState } from "react";
import {
  View,
  FlatList,
  TextInput,
  Pressable,
  Text,
  StyleSheet,
  useWindowDimensions, // 1. Import this
} from "react-native";
import { Picker } from "@react-native-picker/picker";
import { getAuth } from "firebase/auth";
import { deleteObject, ref, getStorage } from "firebase/storage";
import { logActivity } from "../../../services/firestoreReportsAndLogs";
import {
  fetchDestinations,
  addDestination,
  updateDestination,
  archiveDestination,
  unarchiveDestination,
} from "../../../services/firestoreDestinations";
import DestinationItem from "../../../components/common/DestinationItem";
import DestinationModal from "../../../components/common/DestinationModal";
import { DEFAULT_KIND_OPTIONS as KIND_OPTIONS } from "../../../components/common/DestinationModal";

const storage = getStorage();

const INITIAL_FORM_STATE = {
  name: "",
  description: "",
  imageUrl: "",
  cityOrMunicipality: "",
  kind: "",
  tags: [],
  isFeatured: false,
  Coordinates: { latitude: "", longitude: "" },
  contact: { email: "", phoneRaw: "" },
  pricing: {
    lodging: { base: "" },
    mealPlan: {
      breakfastIncluded: false,
      lunchIncluded: false,
      dinnerIncluded: false,
      aLaCarteDefault: "300",
    },
    dayUse: { dayPassPrice: "" },
  },
};

const extractStoragePath = (url) => {
  if (!url) return "";
  try {
    const m = url.match(/\/o\/([^?]+)/);
    return m ? decodeURIComponent(m[1]) : "";
  } catch {
    return "";
  }
};

const toNumOrZero = (v) => {
  const n = parseFloat(v);
  return Number.isFinite(n) ? n : 0;
};

const ManageDestination = () => {
  const auth = getAuth();
  const actorId = auth.currentUser?.uid || "system";
  
  // 2. Get Screen Width
  const { width } = useWindowDimensions();
  const isWide = width > 768; // Breakpoint for Tablets/Desktop

  const [destinations, setDestinations] = useState([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [form, setForm] = useState(INITIAL_FORM_STATE);
  const [editId, setEditId] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [showArchived, setShowArchived] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [saveError, setSaveError] = useState(null);

  const [filterKind, setFilterKind] = useState("");
  const [filterCity, setFilterCity] = useState("");

  const getActorName = () =>
    auth.currentUser?.email || auth.currentUser?.displayName || "system";

  const loadData = async () => {
    const data = await fetchDestinations();
    setDestinations(data);
  };

  useEffect(() => {
    loadData();
  }, []);

  const allKinds = ["", ...new Set(destinations.map((d) => d.kind || ""))];
  const allCities = ["", ...new Set(destinations.map((d) => d.cityOrMunicipality || ""))];

  // ------------------- Save / Create / Update -------------------
  const handleSave = async (incomingForm) => {
    setSaveError(null);
    setIsLoading(true);
    const src = incomingForm ?? form;

    if (!src.name?.trim()) {
      setSaveError("Destination name cannot be empty.");
      setIsLoading(false);
      return;
    }

    const payload = {
      name: src.name.trim(),
      description: src.description?.trim() || "",
      imageUrl: src.imageUrl?.trim() || "",
      cityOrMunicipality: src.cityOrMunicipality?.trim() || "",
      kind: (src.kind || "").toLowerCase(),
      tags: Array.isArray(src.tags) ? src.tags : [],
      isFeatured: !!src.isFeatured,
      Coordinates: {
        latitude: toNumOrZero(src?.Coordinates?.latitude),
        longitude: toNumOrZero(src?.Coordinates?.longitude),
      },
      contact: {
        email: src?.contact?.email || "",
        phoneRaw: src?.contact?.phoneRaw || "",
      },
      pricing: {
        lodging: { base: toNumOrZero(src?.pricing?.lodging?.base) },
        mealPlan: {
          breakfastIncluded: !!src?.pricing?.mealPlan?.breakfastIncluded,
          lunchIncluded: !!src?.pricing?.mealPlan?.lunchIncluded,
          dinnerIncluded: !!src?.pricing?.mealPlan?.dinnerIncluded,
          aLaCarteDefault: toNumOrZero(src?.pricing?.mealPlan?.aLaCarteDefault),
        },
        dayUse: { dayPassPrice: toNumOrZero(src?.pricing?.dayUse?.dayPassPrice) },
      },
      imagePath: src.imagePath || "",
      previousImagePath: src.previousImagePath || "",
    };

    try {
      const actorName = getActorName();

      if (editId) {
        await updateDestination(editId, payload);
        logActivity({
          actorName,
          actorId,
          actionType: "UPDATE",
          targetEntity: "Destination",
          targetId: editId,
          details: `Updated destination: ${payload.name}`,
        });
      } else {
        const newDoc = await addDestination({ ...payload, isArchived: false });
        logActivity({
          actorName,
          actorId,
          actionType: "CREATE",
          targetEntity: "Destination",
          targetId: newDoc.id,
          details: `Created destination: ${payload.name}`,
        });
      }

      try {
        if (payload.previousImagePath && payload.imagePath &&
          payload.previousImagePath !== payload.imagePath) {
          await deleteObject(ref(storage, payload.previousImagePath));
        }
      } catch (e) {
        console.warn("[cleanup] Failed to delete old image:", e);
      }

      setModalVisible(false);
      setForm(INITIAL_FORM_STATE);
      setEditId(null);
      loadData();
    } catch (err) {
      console.error("Firestore Save Error:", err);
      setSaveError("An error occurred while saving.");
    } finally {
      setIsLoading(false);
    }
  };

  const handleEdit = (item) => {
    const existingUrl = item.imageUrl || "";
    const existingPath = extractStoragePath(existingUrl);

    setForm({
      name: item.name || "",
      description: item.description || "",
      imageUrl: existingUrl,
      imagePath: existingPath,
      cityOrMunicipality: item.cityOrMunicipality || "",
      kind: (item.kind || "").toLowerCase(),
      tags: item.tags || [],
      isFeatured: !!item.isFeatured,
      Coordinates: {
        latitude: item?.Coordinates?.latitude?.toString() || "",
        longitude: item?.Coordinates?.longitude?.toString() || "",
      },
      contact: {
        email: item?.contact?.email || "",
        phoneRaw: item?.contact?.phoneRaw || "",
      },
      pricing: {
        lodging: { base: item?.pricing?.lodging?.base?.toString() || "" },
        mealPlan: {
          breakfastIncluded: !!item?.pricing?.mealPlan?.breakfastIncluded,
          lunchIncluded: !!item?.pricing?.mealPlan?.lunchIncluded,
          dinnerIncluded: !!item?.pricing?.mealPlan?.dinnerIncluded,
          aLaCarteDefault: item?.pricing?.mealPlan?.aLaCarteDefault?.toString() || "300",
        },
        dayUse: { dayPassPrice: item?.pricing?.dayUse?.dayPassPrice?.toString() || "" },
      },
    });

    setEditId(item.id);
    setModalVisible(true);
  };

  const handleArchive = async (id, name) => {
    const actorName = getActorName();
    await archiveDestination(id);
    logActivity({
      actorName,
      actorId,
      actionType: "ARCHIVE",
      targetEntity: "Destination",
      targetId: id,
      details: `Archived destination: ${name}`,
    });
    loadData();
  };

  const handleUnarchive = async (id, name) => {
    const actorName = getActorName();
    await unarchiveDestination(id);
    logActivity({
      actorName,
      actorId,
      actionType: "UNARCHIVE",
      targetEntity: "Destination",
      targetId: id,
      details: `Unarchived destination: ${name}`,
    });
    loadData();
  };

  const handleToggleFeatured = async (id, name, value) => {
    const actorName = getActorName();
    await updateDestination(id, { isFeatured: value });
    logActivity({
      actorName,
      actorId,
      actionType: "TOGGLE_FEATURED",
      targetEntity: "Destination",
      targetId: id,
      details: `Feature toggled: ${name} = ${value}`,
    });
    loadData();
  };

  const visibleDestinations = destinations
    .filter((d) => (showArchived ? d.isArchived : !d.isArchived))
    .filter((d) => {
      const q = searchQuery.toLowerCase();
      const matchesSearch =
        d.name?.toLowerCase().includes(q) ||
        (d.cityOrMunicipality || "").toLowerCase().includes(q) ||
        (d.tags || []).join(", ").toLowerCase().includes(q);
      const matchesKind = filterKind ? d.kind?.toLowerCase() === filterKind.toLowerCase() : true;
      const matchesCity = filterCity ? d.cityOrMunicipality?.toLowerCase() === filterCity.toLowerCase() : true;
      return matchesSearch && matchesKind && matchesCity;
    });

  return (
    <View style={styles.container}>
      
      {/* 3. CONTROL BAR CONTAINER */}
      <View style={[styles.controlsContainer, isWide && styles.rowLayout]}>
        
        {/* SEARCH (Flex 2 on wide) */}
        <View style={[styles.controlGroup, isWide && { flex: 2 }]}>
            <Text style={styles.filterLabel}>Search</Text>
            <TextInput
                style={styles.searchInput}
                placeholder="Name, tags, city..."
                placeholderTextColor="#999"
                value={searchQuery}
                onChangeText={setSearchQuery}
            />
        </View>

        {/* FILTER: KIND (Flex 1 on wide) */}
        <View style={[styles.controlGroup, isWide && { flex: 1 }]}>
            <Text style={styles.filterLabel}>Kind</Text>
            <View style={styles.pickerWrapper}>
                <Picker
                    selectedValue={filterKind}
                    onValueChange={setFilterKind}
                    style={styles.picker}
                >
                    <Picker.Item label="All Kinds" value="" />
                    {allKinds.map((k, i) => k && <Picker.Item label={k} value={k} key={i} />)}
                </Picker>
            </View>
        </View>

        {/* FILTER: CITY (Flex 1 on wide) */}
        <View style={[styles.controlGroup, isWide && { flex: 1 }]}>
            <Text style={styles.filterLabel}>City / Municipality</Text>
            <View style={styles.pickerWrapper}>
                <Picker
                    selectedValue={filterCity}
                    onValueChange={setFilterCity}
                    style={styles.picker}
                >
                    <Picker.Item label="All Cities" value="" />
                    {allCities.map((c, i) => c && <Picker.Item label={c} value={c} key={i} />)}
                </Picker>
            </View>
        </View>
      </View>

      {/* ARCHIVE / ACTIVE TOGGLE & ADD BUTTON ROW */}
      <View style={styles.actionRow}>
        <Pressable
            style={[styles.toggleButton, showArchived && styles.toggleActive]}
            onPress={() => setShowArchived(!showArchived)}
        >
            <Text style={[styles.toggleButtonText, showArchived && styles.toggleActiveText]}>
                {showArchived ? "← Back to Active" : "📂 Show Archived"}
            </Text>
        </Pressable>

        {!showArchived && (
            <Pressable
            style={styles.addButton}
            onPress={() => {
                setForm(INITIAL_FORM_STATE);
                setSaveError(null);
                setEditId(null);
                setModalVisible(true);
            }}
            >
            <Text style={styles.addButtonText}>+ Add Destination</Text>
            </Pressable>
        )}
      </View>

      <FlatList
        data={visibleDestinations}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
          <DestinationItem
            item={item}
            onEdit={handleEdit}
            onArchive={() => handleArchive(item.id, item.name)}
            onUnarchive={() => handleUnarchive(item.id, item.name)}
            onToggleFeatured={(v) => handleToggleFeatured(item.id, item.name, v)}
            isArchivedView={showArchived}
          />
        )}
      />

      <DestinationModal
        visible={modalVisible}
        onClose={() => {
          setModalVisible(false);
          setForm(INITIAL_FORM_STATE);
          setEditId(null);
        }}
        onSave={handleSave}
        form={form}
        setForm={setForm}
        editId={editId}
        isLoading={isLoading}
        error={saveError}
        kindOptions={KIND_OPTIONS}
      />
    </View>
  );
};

export default ManageDestination;

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: "#f8f9fa",
    padding: 20,
  },
  // 4. LAYOUT STYLES
  controlsContainer: {
    flexDirection: 'column', // Default Mobile
    gap: 12,
    marginBottom: 15,
  },
  rowLayout: {
    flexDirection: 'row', // Wide Screen
    alignItems: 'flex-end', // Align bottoms (inputs)
  },
  controlGroup: {
    width: '100%',
  },
  // ----------------
  searchInput: {
    borderWidth: 1,
    borderColor: "#ddd",
    borderRadius: 8,
    padding: 10, // Match picker height approx
    height: 45, // Force height to match Picker
    fontSize: 16,
    color: "#333",
    backgroundColor: "#fff",
  },
  filterLabel: {
    fontSize: 14,
    fontWeight: "600",
    color: "#333",
    marginBottom: 6,
  },
  pickerWrapper: {
    borderWidth: 1,
    borderColor: "#ddd",
    borderRadius: 8,
    overflow: "hidden",
    backgroundColor: "#fff",
    height: 45,
    justifyContent: 'center',
  },
  picker: {
    height: 45,
    width: "100%",
    backgroundColor: 'transparent',
    borderWidth: 0, 
  },
  
  // Action Row (Buttons)
  actionRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 15,
  },
  addButton: {
    backgroundColor: "#0f37f1",
    paddingVertical: 10,
    paddingHorizontal: 24,
    borderRadius: 25,
  },
  addButtonText: {
    color: "#fff",
    fontSize: 15,
    fontWeight: "600",
  },
  toggleButton: {
    backgroundColor: "#e2e8f0",
    paddingVertical: 8,
    paddingHorizontal: 18,
    borderRadius: 25,
  },
  toggleButtonText: {
    fontWeight: "600",
    color: "#475569",
  },
  toggleActive: {
    backgroundColor: "#475569",
  },
  toggleActiveText: {
    color: "#fff",
  },
});